...
void cam_req_mgr_core_link_reset(struct cam_req_mgr_core_link *link)
{
	...
	link->recovery_in_progress = false;
	...
	uint32_t trigger)
{
	...
	for (i = 0; i < link->num_devs; i++) {
		...
		if ((link->state == CAM_CRM_LINK_STATE_ERR) || link->recovery_in_progress)
			CAM_INFO(CAM_REQ,
 ...
 *
 */
static int __cam_req_mgr_traverse(struct cam_req_mgr_core_link *link,
	struct cam_req_mgr_traverse *traverse_data)
{
	...
	if (tbl->slot[curr_idx].state == CRM_REQ_STATE_READY ||
		...
		if (tbl->next) {
			...
			rc = __cam_req_mgr_traverse(link, traverse_data);
		}
		if (rc >= 0) {
			...
			if (traverse_data->validate_only == false) {
				...
				if ((link->state == CAM_CRM_LINK_STATE_ERR) ||
					link->recovery_in_progress)
					CAM_INFO(CAM_CRM, "req_id: %lld with pd of %d",
						apply_data[tbl->pd].req_id,
	...
	int32_t idx)
{
	...
	slot = &in_q->slot[idx];
	if (link->recovery_in_progress)
		CAM_INFO(CAM_CRM, "RESET: idx: %d: slot->status %d", idx, slot->status);
	...
	struct cam_req_mgr_core_link *link)
{
	...
	if (link->recovery_in_progress)
		CAM_INFO(CAM_CRM, "idx: %d: slot->status %d", idx, slot->status);
	...
	if (slot->status == CRM_SLOT_STATUS_NO_REQ) {
		...
		if (rc) {
			if (link->recovery_in_progress)
				CAM_INFO(CAM_CRM, "No lower pd devices on link 0x%x",
					link->link_hdl);
			return rc;
		}
	...
	struct cam_req_mgr_trigger_notify *trigger_data)
{
	...
	if (slot->status == CRM_SLOT_STATUS_REQ_APPLIED) {
		CAM_WARN(CAM_CRM, "skipping send req as status is req applied");
		goto end;
	}
	...
	if (rc < 0) {
		...
		if (link->state == CAM_CRM_LINK_STATE_ERR) {
			CAM_WARN(CAM_CRM,
				"Err recovery done idx %d link 0x%x req_id: %lld slot_status: %u",
				in_q->rd_idx, link->link_hdl, in_q->slot[in_q->rd_idx].req_id,
				in_q->slot[in_q->rd_idx].status);
			link->recovery_in_progress = true;
		...
		for (i = 0; i < link->num_devs; i++) {
			...
			if (link->recovery_in_progress)
				CAM_INFO(CAM_CRM,
					"dev %s pd: %d req_id: %lld req_apply_map: 0x%x dev_mask: 0x%x",
					dev->dev_info.name, pd, apply_data[pd].req_id,
		...
		if (((eof_trigger_type == CAM_REQ_EOF_TRIGGER_NONE) ||
			(eof_trigger_type == CAM_REQ_EOF_TRIGGER_APPLIED)) &&
			(trigger == CAM_TRIGGER_POINT_SOF) && is_applied) {
			slot->status = CRM_SLOT_STATUS_REQ_APPLIED;

			if (link->recovery_in_progress) {
				CAM_INFO(CAM_CRM, "req %d is applied on link %x success",
				slot->req_id,
				link->link_hdl);
				link->recovery_in_progress = false;
			}
...
int cam_req_mgr_process_error(void *priv, void *data)
{
	...
	CAM_INFO(CAM_CRM, "link_hdl %x req_id %lld error %d",
		err_info->link_hdl,
		err_info->req_id,
		err_info->error);
...
static int cam_req_mgr_process_trigger(void *priv, void *data)
{
	 ...
	 */
	 if (link->recovery_in_progress)
		CAM_INFO(CAM_CRM, "link[%x] Req[%lld] slot status: %d idx %d",
			link->link_hdl, in_q->slot[in_q->rd_idx].req_id,
			in_q->slot[in_q->rd_idx].status, in_q->rd_idx);
	if (in_q->slot[in_q->rd_idx].status == CRM_SLOT_STATUS_REQ_APPLIED) {
		...
		if (link->recovery_in_progress)
			CAM_INFO(CAM_CRM, "in_q->rd_idx: %d", in_q->rd_idx);
	}
